...
# 启动 DeepSeek Harness Web UI（监听 0.0.0.0:3080，使 ingress 可直接访问）
...
# 使用 --host 0.0.0.0 绑定所有接口，ingress 可直接访问，无需 TCP 代理
DSH_BIN="/usr/local/lib/node_modules/@deepseek-ai/dsh/lib/bin.js"
node --expose-internals "${DSH_BIN}" --profile web --host 0.0.0.0 --port 3080 &
DSH_PID=$!
...
for i in $(seq 1 30); do
    if wget -qO- http://127.0.0.1:3080 > /dev/null 2>&1; then
        echo "[DSH Addon] DSH Web UI is ready on 0.0.0.0:3080"
        break
    fi
    sleep 2
done
